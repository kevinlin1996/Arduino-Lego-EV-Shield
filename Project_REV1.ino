...
void loop() {
  ...
  if (myTouch.isPressed() == true) {
    ...
    uint8_t i;
    uint16_t red_area_total = 0;
    uint16_t red_blob_area = 0;
    uint16_t blue_area_total = 0;
    uint16_t blue_blob_area = 0;
    uint16_t width;
    uint16_t height;
    uint8_t blockIdentity = 0;
    uint8_t movement = 0;
    boolean movementDirection = false; //false = CW, true = CCW
    //0 = 4x1 Grey, 1 = 2x2 Red, 2 = 4x2 Red, 3 = 4x2 Blue
    ...
    if (red_area_total > blue_area_total){
      
      //large red block found
    if (red_area_total > 1500) {
      printString("Big Red block found");
      blockIdentity = 2;
    }

    //small red block found
    else if (red_area_total > 0 || red_area_total < 1500) {
    ...
    else if (blue_area_total > red_area_total){
    ...
    if (blockIdentity == 0) { //1x4 Grey Block, Bottom Cup #1
      evshield.bank_b.motorRunDegrees(SH_Motor_1, SH_Direction_Forward, SH_Speed_Slow, grey1x1, SH_Completion_Wait_For, SH_Next_Action_BrakeHold);
      movement = grey1x1;
